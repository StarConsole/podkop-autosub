...
if [ ! -f /etc/config/podkop_rotator ]; then
    ...
    cp files/etc/config/podkop_rotator /etc/config/podkop_rotator
else
    echo "[*] Конфиг /etc/config/podkop_rotator уже существует, сохраняем пользовательские настройки"
fi

# 2. Копируем исполняемые файлы и выставление прав
echo "[+] Копируем скрипт и init.d сервис..."
cp files/usr/bin/podkop-rotator.sh /usr/bin/podkop-rotator.sh
chmod +x /usr/bin/podkop-rotator.sh

cp files/etc/init.d/podkop_rotator /etc/init.d/podkop_rotator
