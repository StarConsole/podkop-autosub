...
rotate_keys() {
    ...
    curl -sL "$sub_url" > "$TMP_LIST"

    if [ ! -s "$TMP_LIST" ]; then
        log "[ERROR] Failed to download subscription from URL"
        return 1
    fi

    grep -E "^vless://" "$TMP_LIST" | \
    grep -vE "type=(xhttp|splithttp|quic|kcp|http)" | \
    grep -E "security=(tls|reality)" > "$FILTERED_LIST"

    TOTAL_KEYS=$(wc -l < "$FILTERED_LIST")
    log "Subscription downloaded. Valid candidates found: $TOTAL_KEYS"
